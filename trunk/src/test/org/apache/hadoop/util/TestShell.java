...
public class TestShell extends TestCase {
  ...
  public void testDisableShellCommand() throws IOException {
    // Test if we get NO_SHELL_CALL_ALLOWED_STRING if disabled is set.
    Shell.setDisabled(true);
    String out = ShellCommandExecutor.execCommand("pwd");
    assertEquals("Shell calls are not allowed.",
                 Shell.NO_SHELL_CALL_ALLOWED_STRING, out);
    // Now set it back and test it again.
    Shell.setDisabled(false);
    out = ShellCommandExecutor.execCommand("pwd");
    assertFalse("Shell calls should work normal now.",
                out.equals(Shell.NO_SHELL_CALL_ALLOWED_STRING));
  }
