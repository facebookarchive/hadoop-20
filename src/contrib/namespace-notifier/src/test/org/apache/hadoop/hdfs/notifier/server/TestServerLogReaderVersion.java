...
public class TestServerLogReaderVersion {
  ...
  @Test
  public void testPreTransactionalVersion() throws IOException {
    // -37 is pre-transactional layout
    int lv = -37;
    StorageInfo si = new StorageInfo(lv, 10, 0);
    StorageDirectory sd = new NNStorage(si).new StorageDirectory(editsDir);
    ...
    core.setConfiguration(new Configuration());
    core.getConfiguration().set("notifier.edits.dir",
        editsDir.getAbsolutePath());

    // initialize dummy edits
    EditLogFileOutputStream out = null;
    IServerLogReader reader = null;
    try {
      out = TestPreTransactionalServerLogReader.initEdits(editsDir);

      // get the reader
      reader = ServerCore.getReader(core);

      // correct reader is created
      assertTrue(reader instanceof ServerLogReaderPreTransactional);
  ...
  @Test
  public void testTransactionalVersion() throws IOException {
    // -37 is pre-transactional layout
    int lv = -40;
    StorageInfo si = new StorageInfo(lv, 10, 0);
    StorageDirectory sd = new NNStorage(si).new StorageDirectory(editsDir);
    ...
    try {
      ...
      core.getConfiguration()
          .set("notifier.edits.dir", sd.getRoot().toString());
  
      // get the reader
      out = initEdits(editsDir);
      reader = ServerCore.getReader(core);
  
      // correct reader is created
      assertTrue(reader instanceof ServerLogReaderTransactional);
    } finally {
      if (reader != null)
        reader.close();
      if (out != null)
