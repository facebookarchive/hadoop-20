...
public abstract class LoadManager implements Configurable {
  ...
  /**
   * Can a given {@link TaskTracker} run another new task?
   * @param tracker The machine we wish to run a new task on
   * @param job The job from which we want to run a task on this mahcine
   * @param type The type of task that we wnat to run on
