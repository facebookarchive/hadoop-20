...
package org.apache.hadoop.mapred.resourceutilization;
...
/**
 * Cache all the resource utilization information of the {@link Collector}
 * locally. This allows us to obtain the information instantly without waiting
 * the response from Collector via RPC call.
 */
public class CollectorCached implements CollectorProtocol {
  CollectorProtocol rpcCollector = null;
  ...
  private static CollectorCached instance = null;
  public static CollectorCached getInstance(Configuration conf) {
    if (instance == null) {
      synchronized (CollectorCached.class) {
        if (instance == null) {
		      instance = new CollectorCached(conf);
        }
      }
    }
    return instance;
  ...
  private CollectorCached(Configuration conf) {
  ...
  protected void connect() {
    ...
    try {
      ...
      rpcCollector =
            (CollectorProtocol) RPC.getProxy(CollectorProtocol.class,
                                             CollectorProtocol.versionID,
                                             Collector.getAddress(conf),
                                             conf);
    } catch (IOException e) {
  ...
  public long getProtocolVersion(String protocol,
    ...
    if (protocol.equals(CollectorProtocol.class.getName())) {
      return CollectorProtocol.versionID;
    } else {
