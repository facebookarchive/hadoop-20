...
public class TestStandbyStorageRetentionManager {

  public static final Log LOG = LogFactory
      .getLog(TestStandbyStorageRetentionManager.class.getName());
  ...
  public void testBackupWithLimit() throws IOException {
    int limit = 5;
    conf.setInt(StandbyStorageRetentionManager.STANDBY_IMAGE_DAYS_TOKEEP, 1);
    conf.setInt(StandbyStorageRetentionManager.STANDBY_IMAGE_COPIES_TOKEEP,
        limit);
  ...
  public void testSortedListBackups() throws Exception {
    ...
    for (int i = 0; i < backups.length; i++) {
      String b = backups[i];
      backupDates[i] = StandbyStorageRetentionManager.dateForm.parse(b
  ...
  private void testDeleteOldBackupsInternal(int initialCopies,
    ...
    StandbyStorageRetentionManager.deleteOldBackups(dir.getParentFile(),
        backups, daysToKeep, copiesToKeep);

    String[] backupsAfter = StandbyStorageRetentionManager.getBackups(dir);
    ...
    assertEquals(expectedCount,
        StandbyStorageRetentionManager.getBackups(dir).length);
  }
  ...
  private static String[] createBackups(int count, Configuration conf,
    ...
    for (int i = 0; i < count; i++) {
      ...
      try {
        ...
        StandbyStorageRetentionManager.backupFiles(fs, dir, conf);
        if (i == failAtBackup) {
      ...
      String[] backups = StandbyStorageRetentionManager.getBackups(dir);
      assertEquals(i + 1, backups.length);
      sleep();
    }
    return StandbyStorageRetentionManager.getBackups(dir);
  }
