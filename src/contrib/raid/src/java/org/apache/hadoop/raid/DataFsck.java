...
"The input to the utility is a file with the list of files to check.\n"
  ...
  static class DataFsckMapper implements Mapper<Text, Text, Text, Text> {
    ...
    boolean checkAgainstParity(FileSystem fs, FileStatus stat) throws IOException {
      ErasureCodeType code = null;
      int parityLength;
      ParityFilePair ppair = ParityFilePair.getParityFile(
        ErasureCodeType.RS, stat.getPath(), conf);
      if (ppair != null) {
        code = ErasureCodeType.RS;
        parityLength = RaidNode.rsParityLength(conf);
      } else {
        ppair = ParityFilePair.getParityFile(
          ErasureCodeType.XOR, stat.getPath(), conf);
        if (ppair != null) {
    ...
    void computeParity(Configuration conf, FileSystem fs, FileStatus stat,
      ...
      try {
        ...
        if (code == ErasureCodeType.XOR) {
          ...
          int parityLength = RaidNode.rsParityLength(conf);
