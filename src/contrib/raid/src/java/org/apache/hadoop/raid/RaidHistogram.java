...
public class RaidHistogram {
  ...
  public class Point implements Comparable<Point> {
    ...
    public long value;
    public String path;
    public Point(long newValue, String newPath, long newTime) {
      value = newValue;
      path = newPath;
      time = newTime;
    }
  ...
  public RaidHistogram(String[] monitorSeconds, Configuration conf)
      throws Exception {
    ArrayList<Long> windows = new ArrayList<Long>();
    for (int i = 0; i < monitorSeconds.length; i++) {
      windows.add(Long.parseLong(monitorSeconds[i])*1000);
    }
    initialize(windows);
  ...
  public synchronized Set<String> getFailedRecoveredFiles() {
    return Collections.unmodifiableSet(this.failedRecoveredFiles.keySet());
  }
   ...
   */
  public synchronized void put(String path, long value) {
    Point p;
    int last = windowNum - 1;
    if (value == RECOVERY_FAIL) {
      p = new Point(value, path, System.currentTimeMillis());
  ...
  public synchronized void filterStalePoints(long endTime) throws IOException {
    ...
    NavigableSet<Point> windowSet =
        points.headSet(new Point(0, "", endTime - windowTime)); 
  ...
  public synchronized void collectCounters(long endTime) throws IOException {
    for (int i = 0; i < windowNum - 1; i++) {
      ...
      NavigableSet<Point> windowSet =
          points.tailSet(new Point(0, "", endTime - windowTime)); 
      Iterator<Point> windowIterator = windowSet.iterator();
