...
public abstract class BlockIntegrityMonitor extends Configured {
  ...
  private HashMap<String, RaidHistogram> recoveryTimes;
  ...
  public BlockIntegrityMonitor(Configuration conf) throws Exception {
    ...
    for (String monitorDir: monitorDirs) {
      recoveryTimes.put(monitorDir, new RaidHistogram(monitorSeconds, conf));
    }
    recoveryTimes.put(OTHERS, new RaidHistogram(monitorSeconds, conf));
  ...
  public void sendRecoveryTime(String path, long recoveryTime) throws IOException {
    ...
    for (String monitorDir: monitorDirs) {
      if (path.startsWith(monitorDir)) {
        match = true;
        recoveryTimes.get(monitorDir).put(path, recoveryTime);
      }
    }
    if (match == false) {
      recoveryTimes.get(OTHERS).put(path, recoveryTime); 
    }
  }
