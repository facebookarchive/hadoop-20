...
int hdfsGetNumCurrentReplicas(hdfsFS fs, hdfsFile f)
{
    // JAVA EQUIVALENT
    // ((DFSOutputStream)FSDataOutputStream.getWrappedStream()).getNumCurrentReplicas
    ...
    if (invokeMethod(env, &jVal, &jExc, INSTANCE, jOutputStream,
                     HADOOP_OSTRM, "getWrappedStream", "()Ljava/io/OutputStream;") != 0) {
        errno = errnoFromException(jExc, env, "org.apache.hadoop.fs."
                                   "FSDataOutputStream::getWrappedStream");
        return -1;
    }
    
    jobject jWrappedStream = (jobject)(jVal.l);

    if (invokeMethod(env, &jVal, &jExc, INSTANCE, jWrappedStream,
                     "org/apache/hadoop/hdfs/DFSClient$DFSOutputStream", 
                     "getNumCurrentReplicas", "()I") != 0) {
        errno = errnoFromException(jExc, env, "org.apache.hadoop.hdfs.DFSClient"
                                   "DFSOutputStream::getNumCurrentReplicas");
        destroyLocalReference(env, jWrappedStream);
