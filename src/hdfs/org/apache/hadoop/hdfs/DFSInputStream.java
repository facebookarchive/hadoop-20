...
public class DFSInputStream extends FSInputStream {
  ...
  public synchronized int available() throws IOException {
    ...
    if (timeShowNoAvailableByte != NOT_WAITING_FOR_MORE_BYTES
      ...
      // To avoid the minor case that we stick to a datanode with
      // a replica with older generation stamp, after available stays
      // to be zero for a while, we re-fetch all the metadata from
      // the name node.
      //
    ...
    return (int) (Math.min(availableBytes, (long) Integer.MAX_VALUE));
  }
