...
public class HftpFileSystem extends FileSystem {
  ...
  public static final String STRICT_CONTENT_LENGTH = "dfs.hftp.strictContentLength";

  private boolean doStrictContentLengthCheck = false;
  ...
  public FSDataInputStream open(Path f, int buffersize) throws IOException {
    HftpInputStream hftpIn = new HftpInputStream(f);
    
    return new FSDataInputStream(hftpIn) {
      @Override
      public boolean isUnderConstruction() {
        return ((HftpInputStream)in).isUnderConstruction();
      }
    };
  }

  private class HftpInputStream extends FSInputStream {
    ...
    public HftpInputStream(Path file) throws IOException {
      this.file = file;
      openStream("/data" + file.toUri().getPath(), "ugi=" + ugi);
      pos = 0;
    }
      ...
      if (ret >= 0) {
        ...
        totalBytesRead++;
          ...
          throw new EOFException(
            String.format(
              "premature end of stream: expected total %d bytes and got only %d",
              pos, contentLength
            ));
    ...
    public void seek(long pos) throws IOException {
      in.close();
      openStream("/data" + file.toUri().getPath(),
                 "seek=" + pos + "&ugi=" + ugi);
      this.pos = pos;
    }
    ...
    private void openStream(String path, String query) throws IOException {
      ...
      String contentLengthStr = 
        connection.getHeaderField(HftpFileSystem.CONTENT_LENGTH_FIELD);
      
      totalBytesRead = 0;
      
      if (contentLengthStr != null) {
        contentLength = Long.valueOf(
          contentLengthStr
        );
      } else {
        if (HftpFileSystem.this.doStrictContentLengthCheck) {
          throw new IOException("missing require Content-Length header");
        } else {
      ...
      isUnderConstruction =
          "true".equals(connection.getHeaderField("isUnderConstruction"));
