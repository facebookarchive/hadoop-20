...
public class FSNamesystem implements FSConstants, FSNamesystemMBean, FSClusterStats {
  ...
  private LocatedBlocks getBlockLocationsInternal(String src,
    ...
    if (doAccessTime && isAccessTimeSupported()) {
      // we lock the inode so that we are guaranteed that access times are
      // always consistent and increasing. 
      synchronized (inode) {
   ...
   */
  public synchronized boolean abandonFile(String src, String holder
      ) throws IOException {
  ...
  /**
   * corrupts a file by:
   *  1. assigning the lease to the NN
   *  2. removing all targets of the last block
   */
  void corruptFileForTesting(String src) throws IOException {
    LeaseManager.Lease lease = leaseManager.getLeaseByPath(src);

    if (lease != null) {
      leaseManager.reassignLease(
        lease, src, HdfsConstants.NN_RECOVERY_LEASEHOLDER
      );
      INodeFile inode = dir.getFileINode(src);
      ...
      if (inode.isUnderConstruction()) {
        ...
        pendingFile.setLastBlock(
          new BlockInfo(
            pendingFile.getLastBlock(), pendingFile.getReplication()
          ), 
  ...
  /**
   * @return map from inodes to corrupt blocks, with a maximum of 
   * MAX_CORRUPT_FILES_RETURNED inodes listed in total
   */
  private INode[] getCorruptInodes() {
  ...
  /**
   * @return Map from FileStatus to LocatedBlocks object with locations of 
   * corrupted blocks.
   * @throws AccessControlException
   * @throws IOException
   */
