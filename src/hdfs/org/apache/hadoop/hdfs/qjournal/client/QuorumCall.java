...
class QuorumCall<KEY, RESULT> {
  ...
  static <KEY, RESULT> QuorumCall<KEY, RESULT> create(
    ...
    for (final Entry<KEY, ? extends ListenableFuture<RESULT>> e : calls.entrySet()) {
      if (e.getValue() == null) {
        throw new IllegalArgumentException("null future for key: " + e.getKey());
      }
  ...
  public synchronized void waitFor(
    ...
    while (true) {
      ...
      if ((maxExceptions > 0 && countExceptions() >= maxExceptions)
          || (maxExceptions == 0 && countExceptions() > 0)) {
        return;
      }
      ...
      if (now > nextLogTime) {
        ...
        if (!exceptions.isEmpty()) {
          msg += ". Exceptions so far: [" + mapToString(exceptions) + "]"; 
        }
  ...
  public synchronized Map<KEY, RESULT> getAllResults() {
    return successes;
  }
