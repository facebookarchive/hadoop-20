...
public class TestQuorumJournalManager {
  ...
  public void testFormat() throws Exception {
    QuorumJournalManager qjm = new QuorumJournalManager(conf,
        cluster.getQuorumJournalURI("testFormat-jid"), FAKE_NSINFO, null, false);
    assertFalse(qjm.hasSomeJournalData());
    qjm.transitionJournal(FAKE_NSINFO, Transition.FORMAT, null);
    assertTrue(qjm.hasSomeJournalData());
    
    assertFalse(qjm.hasSomeImageData());
    qjm.transitionImage(FAKE_NSINFO, Transition.FORMAT, null);
    assertTrue(qjm.hasSomeImageData());
  }
  ...
  @Test
  public void testSetCommittedTxIdForce() throws Exception {
    setupLoggers345();
    ...
    qjm.setCommittedTxId(88888887, false);
    // Can set committed tx if moving forward
    qjm.setCommittedTxId(88888888, false);

    // Can set committed tx if moving backward with force mode
    qjm.setCommittedTxId(66666666, true);

    // Cannot set committed tx backward if not in force mode
    boolean needFail = false;
    try {
      qjm.setCommittedTxId(66666665, false);
      needFail = true;
    } catch (Exception e) {
      e.printStackTrace();
    }
    assertFalse(needFail);

    // Make sure committed tx ID is moved backward to 66666666
    qjm.setCommittedTxId(66666667, false);
  }
  ...
  private void setupEdgeCaseOneJnHasSegmentWithAcceptedRecovery() throws Exception {
    ...
    futureThrows(new IOException("mock failure")).when(spies.get(1))
      .acceptRecovery(Mockito.<SegmentStateProto>any(), Mockito.<String>any());
    ...
    // method javadoc.
    GenericTestUtils.assertGlobEquals(cluster.getJournalCurrentDir(0, JID),
        "edits_.*",
        NNStorage.getFinalizedEditsFileName(1, 100),
        NNStorage.getInProgressEditsFileName(101));
    GenericTestUtils.assertGlobEquals(cluster.getJournalCurrentDir(1, JID),
        "edits_.*",
        NNStorage.getFinalizedEditsFileName(1, 100),
        NNStorage.getInProgressEditsFileName(101) + ".empty");
    GenericTestUtils.assertGlobEquals(cluster.getJournalCurrentDir(2, JID),
        "edits_.*",
        NNStorage.getFinalizedEditsFileName(1, 100),
        NNStorage.getInProgressEditsFileName(101) + ".empty");

    File paxos0 = new File(cluster.getJournalCurrentDir(0, JID), "paxos");
    File paxos1 = new File(cluster.getJournalCurrentDir(1, JID), "paxos");
    File paxos2 = new File(cluster.getJournalCurrentDir(2, JID), "paxos");
  ...
  public void testPurgeLogs() throws Exception {
    ...
    File curDir = cluster.getJournalCurrentDir(0, JID);
  ...
  public void testToString() throws Exception {
    assertTrue(
        qjm.toString().matches("QJM to \\[127.0.0.1:\\d+, 127.0.0.1:\\d+, 127.0.0.1:\\d+\\]"));
  }
  
  private QuorumJournalManager createSpyingQJM() throws IOException,
      URISyntaxException {
    return createSpyingQJM(conf, cluster, JID, FAKE_NSINFO);
  }
