...
public class TestBlockPlacementPolicyHBase {
  ...
  private static final String[] hosts = { "host1", "host2", "host3", "host4", "host5", "host6", };
  ...
  public void setUp() throws IOException {
    ...
    for (int i = 0; i < hosts.length; i++) {
      StaticMapping.addNodeToRack(hosts[i], locations[i]);
    }

    cluster = new MiniDFSCluster(conf, locations.length, locations, hosts, true, true);
  ...
  public void testAndReportDeletionPolicy() throws IOException, InterruptedException {
    ...
    LOG.info("Favor Nodes: " + Arrays.toString(hosts));
    LOG.info("Node Stoped: " + hosts[nodeStop]);
    LOG.info(situation1);
    LOG.info(situation2);
    LOG.info("LOG DONE!");
  ...
  private DatanodeInfo getDataNodeInfo(int nodeIndex) throws IOException {
    for (DatanodeInfo dataNodeInfo : nameNode.getDatanodeReport(DatanodeReportType.ALL)) {
      if (dataNodeInfo.getHostName().equals(hosts[nodeIndex])) {
        return dataNodeInfo;
      }
    }
    return null;
