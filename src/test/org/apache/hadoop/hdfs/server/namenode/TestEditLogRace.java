...
public class TestEditLogRace {
  ...
  private static final String NAME_DIR =
      new File(System.getProperty("test.build.data"), "name1").getAbsolutePath();
    ...
    public void run() {
      ...
      while (!stopped) {
        try {
          ...
          namesystem.mkdirs(dirname, new PermissionStatus("test", "Test",
  ...
  private static void login(){
    try {
      LOG.info("setting the user");
      UserGroupInformation.setCurrentUser(UnixUserGroupInformation.login(new Configuration()));
  ...
  public void testEditLogRolling() throws Exception {
    ...
    try {
      ...
      StorageDirectory sd = fsimage.storage.getStorageDir(0);

      startTransactionWorkers(namesystem, caughtErr);

      long previousLogTxId = 0;
  ...
  private long verifyEditLogs(FSNamesystem namesystem, FSImage fsimage, 
    ...
    for (StorageDirectory sd :
      fsimage.storage.dirIterable(NameNodeDirType.EDITS)) {
      ...
      LOG.info("num edits: " + numEdits + " this log: " + numEditsThisLog);
  ...
  public void testSaveNamespace() throws Exception {
    ...
    try {
      ...
      for (int i = 0; i < NUM_SAVE_IMAGE && caughtErr.get() == null; i++) {
        ...
        long logStartTxId = fsimage.storage.getMostRecentCheckpointTxId() + 1; 
        ...
        namesystem.saveNamespace(false, false);
        long savedImageTxId = fsimage.storage.getMostRecentCheckpointTxId();
        ...
        assertEquals(fsimage.storage.getMostRecentCheckpointTxId(),
    ...
    try {
      ...
      Answer<Void> blockingFlush = new Answer<Void>() {
        ...
        public Void answer(InvocationOnMock invocation) throws Throwable {
          ...
          if (Thread.currentThread().equals(doAnEditThread)) {
  ...
  public void testSaveRightBeforeSync() throws Exception {
    ...
    try {
      ...
      doAnswer(blockingSync).when(editLog).logSync(anyBoolean());
