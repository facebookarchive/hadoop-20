...
/**
 * Unit test to verify that the pure-Java CRC32 and NativeCRC32 algorithm gives
 * the same results as the built-in implementation.
 */
public class TestCustomCrc32 {
  private final CRC32 theirs = new CRC32();
  private final PureJavaCrc32 pureJava = new PureJavaCrc32();
  private final NativeCrc32 nativeCrc = new NativeCrc32();
  ...
  private void checkOnBytes(byte[] bytes, boolean print) {
    ...
    for (int i = 0; i < bytes.length; i++) {
      nativeCrc.update(bytes[i]);
      pureJava.update(bytes[i]);
    ...
    if (print) {
      System.out.println("theirs:\t" + Long.toHexString(theirs.getValue())
          + "\npureJava:\t" + Long.toHexString(pureJava.getValue())
          + "\nnativeCrc:\t" + Long.toHexString(nativeCrc.getValue()));
    }
    ...
    pureJava.update(bytes, 0, bytes.length);
    theirs.update(bytes, 0, bytes.length);
    nativeCrc.update(bytes, 0, bytes.length);
    if (print) {
      System.out.println("theirs:\t" + Long.toHexString(theirs.getValue())
          + "\npureJava:\t" + Long.toHexString(pureJava.getValue())
          + "\nnativeCrc:\t" + Long.toHexString(nativeCrc.getValue()));
    ...
    if (bytes.length >= 10) {
      pureJava.update(bytes, 5, 5);
      theirs.update(bytes, 5, 5);
      nativeCrc.update(bytes, 5, 5);
      checkSame();
    }
  }

  private void checkSame() {
    Assert.assertEquals(theirs.getValue(), pureJava.getValue());
    Assert.assertEquals(theirs.getValue(), nativeCrc.getValue());
  }
   ...
   *      'org.apache.hadoop.util.TestCustomCrc32$PerformanceTest' <-native>
   *
   *   -native option for NativeCrc32
   *
    ...
    public static void main(String args[]) {
      if (args.length == 1) {
        if (args[0].equals("-native")) {
          // We don't run PureJavaCrc32 and NativeCrc32 together since for
          // some reason that I don't understand fully running them together
          // causes an artifical boost to the PureJavaCrc32 perf.
          CRCS[0] = new NativeCrc32();
        } else {
