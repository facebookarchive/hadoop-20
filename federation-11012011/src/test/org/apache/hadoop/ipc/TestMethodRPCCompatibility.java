...
public class TestMethodRPCCompatibility {
  private static final String ADDRESS = "0.0.0.0";
  private static InetSocketAddress addr;
  private static Server server;
  private ProtocolProxy<? extends VersionedProtocol> proxy;
  ...
  public void testVersion0ClientVersion1Server() throws Exception {
    ...
    server = RPC.getServer(new TestImpl1(), ADDRESS, 0, 2, false, conf);
    server.start();
  ...
  public void testVersion1ClientVersion0Server() throws Exception {
    // create a server with two handlers
    server = RPC.getServer(new TestImpl0(), ADDRESS, 0, 2, false, conf);
    server.start();
    addr = NetUtils.getConnectAddress(server);
  ...
  public void testVersion2ClientVersion2Server() throws Exception {
    // create a server with two handlers
    server = RPC.getServer(new TestImpl2(), ADDRESS, 0, 2, false, conf);
    server.start();
    addr = NetUtils.getConnectAddress(server);
