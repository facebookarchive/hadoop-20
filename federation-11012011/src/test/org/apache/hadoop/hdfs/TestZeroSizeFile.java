...
public class TestZeroSizeFile extends junit.framework.TestCase {
  ...
  public void testRecoverZeroChecksumFile() throws Exception {
    ...
    DatanodeInfo[] nodes = (DatanodeInfo[]) nextBlockOutputStreamMethod.invoke(
        out.getWrappedStream(), dfs.dfs.getClientName());
    ...
    meta.delete();
    DataOutputStream outs = new DataOutputStream(new FileOutputStream(metaPath,
        false));
    outs.close();
    
    // issue recovery and make sure it succeeds.
    int numTries = 500;
    ...
    locatedblock = TestInterDatanodeProtocol
        .getLastLocatedBlock(dfs.dfs.namenode, filestr);
    ...
    InputStream in = dfs.open(new Path(filestr), 0);
