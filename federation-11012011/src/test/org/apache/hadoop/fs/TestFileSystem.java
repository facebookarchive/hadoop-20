...
public class TestFileSystem extends TestCase {
  ...
  /**
   * A FileSystem that is slow in creation. Used for testing FileSystem Cache
   * with multi-threaded access. 
   */
  public static class SlowCreationFileSystem extends FilterFileSystem {
    
    public static final int delayMS = 3 * 1000;
    SlowCreationFileSystem() throws IOException {
      super(FileSystem.getLocal(conf));
      try {
  ...
  public void testFsCacheMultiThreaded() throws Exception {
    doTestFsCacheMultiThreaded(true);
    doTestFsCacheMultiThreaded(false);
  }

  public void doTestFsCacheMultiThreaded(final boolean unique) throws Exception {
    final Configuration conf = new Configuration();
    conf.set("fs.slowcreation.impl", SlowCreationFileSystem.class.getName());
    conf.set("fs.default.name", "slowcreation://localhost/");
    
    // Create 3 Threads which will create 3 FileSystems using the same conf.
    final int n = 3;
    final FileSystem[] fs = new FileSystem[n];
    ...
    for (int i = 0; i < n; i++) {
      ...
      threads[ti] = new Thread() {
        ...
        public void run() {
          try {
            fs[ti] = unique ? FileSystem.newInstance(conf) : FileSystem.get(conf);
          } catch (IOException e) {
            // Ignored
          }
        }
    ...
    // Verify that they are unique (or not unique).
    for (int i = 1; i < n; i++) {
      assertTrue(unique == (fs[0] != fs[i]));
      assertTrue(fs[i] instanceof SlowCreationFileSystem);
    }
    
    // The total creation time should be only a bit more than creation of 
    // a single FileSystem object.
    assertTrue((finishMS - startMS) < 1000 + SlowCreationFileSystem.delayMS);
  }
