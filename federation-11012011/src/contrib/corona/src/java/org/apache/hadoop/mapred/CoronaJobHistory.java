...
import org.apache.hadoop.mapred.JobHistory.CloseWriters;
import org.apache.hadoop.mapred.JobHistory.Keys;
import org.apache.hadoop.mapred.JobHistory.KeyValuePair;
...
public class CoronaJobHistory {
  ...
  public CoronaJobHistory(Configuration conf, JobID jobId) {
    try {
      this.conf = conf;
      this.jobId = jobId;
      logDir = new Path
      ...
      if (doneLocation != null) {
        doneDir = new Path(doneLocation);
        doneDirFs = doneDir.getFileSystem(conf);
      } else {
      ...
      fileManager = new JobHistory.JobHistoryFilesManager
        (conf, 
